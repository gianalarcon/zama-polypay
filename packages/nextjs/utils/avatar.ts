...
export const getAvatarByAccountId = (accountId?: string | null): string => {
  if (!accountId) return ACCOUNT_AVATARS[0];
  let hash = 0;
...
export const getAvatarByCommitment = (commitment?: string | null): string => {
  if (!commitment) return USER_AVATARS[0];
  let hash = 0;
