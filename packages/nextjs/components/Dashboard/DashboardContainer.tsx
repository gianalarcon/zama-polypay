...
/**
 * Polypay-Zama Dashboard.
 *
 * Original Polypay UI verbatim: a header with InfoCardContainer (account /
 * balance / pending stats) and an infinite-scroll transaction list rendered
 * by TransactionRow. The data source is now the Zama relayer
 * (/api/zama/accounts/:address/proposals) — the transactionApi adapter
 * shapes proposals into the Polypay Transaction type so this container
 * doesn't need to know it's running on FHE-encrypted state under the hood.
 */
function Header() {
  return (
