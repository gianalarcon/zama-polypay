...
Run these from the repo root, top to bottom. The only manual edit is pasting your relayer private key into the backend `.env` file in step 5.
...
#    (backend + frontend import compiled artifacts; this step is required
#    on a fresh clone because shared/dist is not committed)
...
# 6. Apply DB migrations (also runs `prisma generate`)
yarn workspace @polypay-zama/backend prisma:migrate
```

> Frontend has no `.env` — Sepolia public RPC, hUSD address, and demo Wallet-Connect project ID all default in code.
...
That's it — open `http://localhost:3000` and follow the [end-to-end demo](#end-to-end-demo) below.
...
This builds `@polypay-zama/backend` and `@polypay-zama/frontend` into their respective `dist/` and `.next/` directories. (`@polypay/shared` was already built in step 3.)

### Deploying your own contracts (optional)

Skip this unless you need a fresh hUSD instance. You'll need `packages/hardhat/.env`:
